...
describe('update an land title', async () => {
  let app: Express;
  ...
  beforeEach(async () => {
    ...
    authorizedUser = await testService.createAuthUserAndGetAccessToken({
      permissions: ['land-titles:update'],
    });
  ...
  it('E.1. fails when the user is not authenticated', async () => {
    const resultLandTitleFactory = await new LandTitleFactory(DatabaseTestUtil.dbConnection).create();

    const response = await request(app)
      .patch(`/v1/master/land-titles/${resultLandTitleFactory.inserted_id}`)
  ...
  it('E.4. fails when a unique database field already exists', async () => {
    ...
    await landTitleFactory.state({ name: nameDuplicate }).create();

    // seed data to edit the name to the same name as above
    const resultLandTitleFactory = await landTitleFactory.state({ name: faker.person.fullName() }).create();
  ...
  it('S.1. succeeds', async () => {
    const resultLandTitleFactory = await new LandTitleFactory(DatabaseTestUtil.dbConnection).createMany(2);
    ...
    const response = await request(app)
      .patch(`/v1/master/land-titles/${resultLandTitleFactory.inserted_ids[0]}`)
      .set('Authorization', `Bearer ${authorizedUser.accessToken}`)
      .send(updateData);
    ...
    expect(unmodifiedLandTitleRecord?.name).toStrictEqual(landTitles.data[1].name);
  });
});
