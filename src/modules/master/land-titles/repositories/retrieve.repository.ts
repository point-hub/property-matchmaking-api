...
export interface IRetrieveRepository {
  ...
  raw(_id: string): Promise<ILandTitle | null>
}
...
export class RetrieveRepository implements IRetrieveRepository {
  ...
  async raw(_id: string): Promise<ILandTitle | null> {
    const response = await this.database.collection(collectionName).retrieve<ILandTitle>(_id, this.options);
    if (!response) {
      return null;
    }
